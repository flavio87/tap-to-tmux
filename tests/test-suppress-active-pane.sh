#!/bin/bash
# Tests for PR #3: suppress notifications when tmux pane is actively focused
...
# If running inside tmux, extract the socket path for commands
TMUX_SOCKET=""
if [[ -n "${TMUX:-}" ]]; then
...
# Create a 2-pane test session and set PANE_A / PANE_B to actual indices
setup_session() {
    tmux_cmd new-session -d -s "$TEST_SESSION" -x 120 -y 40
    tmux_cmd split-window -t "$TEST_SESSION" -h
    sleep 0.3
    # Discover the actual pane indices
...
should_suppress() {
    local suppress_config="$1" session="$2" pane_idx="$3"
    ...
    if [[ "${suppress_config:-none}" == "pane" && -n "$session" && -n "$pane_idx" ]]; then
        ...
        _pane_state=$(tmux_cmd list-panes -t "${session}" -s \
            ...
            | awk -v idx="$pane_idx" '$1 == idx {print $2, $3}')
...
result=$(should_suppress "none" "$TEST_SESSION" "$PANE_A")
assert_eq "$result" "notify" "none config, active pane => notify"

result=$(should_suppress "" "$TEST_SESSION" "$PANE_A")
assert_eq "$result" "notify" "empty config, active pane => notify"

result=$(should_suppress "none" "$TEST_SESSION" "$PANE_B")
...
result=$(should_suppress "pane" "$TEST_SESSION" "$PANE_A")
...
result=$(should_suppress "pane" "$TEST_SESSION" "$PANE_B")
...
# Pane A in window 0 is the "selected pane" of that window,
# but the window itself is not active (we're on window 1 now)
result=$(should_suppress "pane" "$TEST_SESSION" "$PANE_A")
...
tmux_cmd select-pane -t "${TEST_SESSION}:.${PANE_A}"
sleep 0.1
result_a=$(should_suppress "pane" "$TEST_SESSION" "$PANE_A")
result_b=$(should_suppress "pane" "$TEST_SESSION" "$PANE_B")
