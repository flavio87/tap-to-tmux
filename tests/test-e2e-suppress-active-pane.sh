#!/bin/bash
# E2E tests for suppress notifications when tmux pane is actively focused
#
...
# Args: session window_idx pane_idx suppress_config result_file
run_hook_in_pane() {
    local session="$1" window_idx="$2" pane_idx="$3" suppress_config="$4" result_file="$5"
...
WRAPPER
    ...
    tmux_cmd send-keys -t "${session}:${window_idx}.${pane_idx}" \
...
echo "=== E2E Tests: Suppress Active Pane ==="
...
run_hook_in_pane "$TEST_SESSION" "$WINDOW_A" "$PANE_A" "" "$RESULT_FILE"
...
tmux_cmd select-pane -t "${TEST_SESSION}:${WINDOW_A}.${PANE_A}"
sleep 0.1

RESULT_FILE="${WORK_DIR}/result_test2"
run_hook_in_pane "$TEST_SESSION" "$WINDOW_A" "$PANE_A" "pane" "$RESULT_FILE"
outcome=$(was_notification_sent "$RESULT_FILE")
...
tmux_cmd select-pane -t "${TEST_SESSION}:${WINDOW_A}.${PANE_B}"
sleep 0.1
...
# Even though pane A is the "selected" pane in WINDOW_A, the window itself is inactive
# Use full target session:window.pane to reach the correct pane
...
run_hook_in_pane "$TEST_SESSION" "$WINDOW_A" "$PANE_A" "none" "$RESULT_FILE"
outcome=$(was_notification_sent "$RESULT_FILE")
if [[ "$outcome" == "sent" ]]; then
